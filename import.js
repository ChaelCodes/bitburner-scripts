...
export async function main(ns) {
    ...
    if (filesImported) {
        ...
        ns.tprint(`A good place to start is running \`run /${config.folder}/dashboard.js\``);
    } else {
...
async function importFiles(ns) {
    let files = ['scripts/autoHack.js', 'scripts/autoRemoteHack.js', 'scripts/buyHacknet.js', 'scripts/hack.js', 'scripts/dashboard.js', 'scripts/purchaseServers.js', 'scripts/remoteHack.js'];
    let filesImported = true;
    for (let file of files) {
        let result = await ns.wget((config.rootUrl + file), `/${config.folder}/${file}`);
